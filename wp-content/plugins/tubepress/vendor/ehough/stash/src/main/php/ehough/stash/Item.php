...
class ehough_stash_Item implements ehough_stash_interfaces_ItemInterface
{
     ...
     */
    protected function setDriver(ehough_stash_interfaces_DriverInterface $driver)
    {
        $this->driver = $driver;
    }
    ...
    protected function validateRecord($validation, &$record)
    {
        if (is_array($validation)) {
            ...
            $invalidation = ehough_stash_Invalidation::NONE;
        }
        ...
        if (isset($record['expiration']) && ($ttl = $record['expiration'] - $curTime) > 0) {
            ...
            if ($invalidation == ehough_stash_Invalidation::PRECOMPUTE) {
        ...
        if (!isset($invalidation) || $invalidation == ehough_stash_Invalidation::NONE) {
            $this->isHit = false;
        ...
        switch ($invalidation) {
            case ehough_stash_Invalidation::VALUE:
                if (!isset($arg)) {
                    $this->isHit = false;
            ...
            case ehough_stash_Invalidation::SLEEP:
                ...
                usleep($ptime);
                $record['data']['return'] = $this->get(ehough_stash_Invalidation::SLEEP, $time, $attempts - 1);
                break;

            case ehough_stash_Invalidation::OLD:
                $this->isHit = true;
                break;

            default:
            case ehough_stash_Invalidation::NONE:
                $this->isHit = false;
                break;
        } // switch($invalidate)
    }
