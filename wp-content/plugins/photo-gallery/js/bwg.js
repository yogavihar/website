...
function spider_ajax_save(form_id, tr_group) {
  ...
  var gallery_source = jQuery("#gallery_source").val();
  var autogallery_image_number = jQuery("#autogallery_image_number").val();
  var update_flag = jQuery("input[name=update_flag]:checked").val();
  var instagram_post_gallery =  jQuery("input[name=instagram_post_gallery]:checked").val();
  var gallery_type = gallery_type_input + (instagram_post_gallery == 1 ? "_post" : "");
  ...
  post_data["instagram_post_gallery"] = instagram_post_gallery;
...
function bwg_change_theme_type(type) {
  ...
  jQuery("#type_Carousel").attr("style", "background-color: #F4F4F4;");
...
function bwg_change_gallery_type(type_to_set, warning_type, instagram_client_id) {
  ...
  if(type_to_set == 'instagram'){
    ...
    if(instagram_client_id == ''){ 
      alert('You do not have Instagram CLIENT_ID. Input its value in Options->Embed options. ');
...
function bwg_check_instagram_gallery_input(instagram_client_id, from_popup){
  ...
  if(from_popup){
    ...
    if(jQuery('#popup_instagram_image_number').val() > 33 ||jQuery('#popup_instagram_image_number').val() < 1 ){ 
      ...
      alert('Instagram recent post number must be between 1 and 33.');
...
function bwg_check_gallery_empty(reset, message){
  ...
  if(tr_count != 0){
    if(reset){
      if(message){
        alert("The gallery is not empty. Please delete all the images first.");
      }
      jQuery('#gallery_type').val('');
      jQuery('#tr_instagram_post_gallery').hide();
      jQuery('#tr_gallery_source').hide();
      ...
      if(message){
        alert("The gallery is not empty. Please reset gallery type and delete all the images first.");
      }
    }
    return false;
  }
