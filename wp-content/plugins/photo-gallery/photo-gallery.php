...
if ($wpdb->query("SHOW TABLES LIKE '" . $wpdb->prefix . "bwg_option'")) {
  $bwg_options_row = $wpdb->get_row($wpdb->prepare('SELECT images_directory,permissions FROM ' . $wpdb->prefix . 'bwg_option WHERE id="%d"', 1));
  $WD_BWG_UPLOAD_DIR = $bwg_options_row->images_directory . '/photo-gallery';
  $bwg_permissions = $bwg_options_row->permissions;
  if ($bwg_permissions != 'moderate_comments' && $bwg_permissions != 'publish_posts' && $bwg_permissions != 'edit_posts') {
    $bwg_permissions = 'manage_options';
  }
}
else {
...
function bwg_options_panel() {
  global $bwg_permissions;
  $galleries_page = add_menu_page('Photo Gallery', 'Photo Gallery', $bwg_permissions, 'galleries_bwg', 'bwg_gallery', WD_BWG_URL . '/images/best-wordpress-gallery.png');

  $galleries_page = add_submenu_page('galleries_bwg', 'Add Galleries/Images', 'Add Galleries/Images', $bwg_permissions, 'galleries_bwg', 'bwg_gallery');
  ...
  $albums_page = add_submenu_page('galleries_bwg', 'Albums', 'Albums', $bwg_permissions, 'albums_bwg', 'bwg_gallery');
  ...
  $tags_page = add_submenu_page('galleries_bwg', 'Tags', 'Tags', $bwg_permissions, 'tags_bwg', 'bwg_gallery');
  ...
  $comments_page = add_submenu_page('galleries_bwg', 'Comments', 'Comments', 'manage_options', 'comments_bwg', 'bwg_gallery');
  ...
  $rates_page = add_submenu_page('galleries_bwg', 'Ratings', 'Ratings', 'manage_options', 'rates_bwg', 'bwg_gallery');
...
function bwg_filemanager_ajax() {
  global $bwg_permissions;
  if (function_exists('current_user_can')) {
    if (!current_user_can($bwg_permissions)) {
      die('Access Denied');
    }
  }
  else {
...
function bwg_add_embed_ajax() {
  ...
  if (($embed_action != '') && ($embed_action == 'addInstagramGallery')) {
    $instagram_user = WDWLibrary::get('instagram_user');
    $instagram_client_id = WDWLibrary::get('instagram_client_id');
    $autogallery_image_number = WDWLibrary::get('autogallery_image_number');
    $whole_post = WDWLibrary::get('whole_post');
    $data = WDWLibraryEmbed::add_instagram_gallery($instagram_user, $instagram_client_id, $whole_post, $autogallery_image_number);
...
function bwg_ajax() {
  ...
  $page = WDWLibrary::get('action');
  if (function_exists('current_user_can')) {
    if (!current_user_can($page == 'BWGShortcode' ? 'manage_options' : $bwg_permissions)) {
      die('Access Denied');
    }
  }
  else {
...
function bwg_front_end_scripts() {
  ...
  wp_enqueue_script('bwg_raty', WD_BWG_FRONT_URL . '/js/jquery.raty.js', array(), '2.5.2');
  wp_enqueue_script('bwg_featureCarousel', WD_BWG_URL . '/js/jquery.featureCarousel.js', array(), $version);
...
function bwg_add_autoupdate_interval( $schedules ) {
  ...
  $schedules['bwg_autoupdate_interval'] = array(
    ...
    'display' => __( 'Photo gallery plugin autoupdate interval.' )
  );
  return $schedules;
}


add_action( 'bwg_schedule_event_hook', 'bwg_instagram_galleries' );
...
function bwg_instagram_galleries() {
  ...
  if(!empty($instagram_galleries[0]))
  {
    foreach ($instagram_galleries as $gallery) {
      array_push($response, WDWLibraryEmbed::refresh_instagram_gallery($gallery));
    }
  }
  wp_die();
}
