    ...
    foreach ($images_new as $image_new) {
      ...
      if($to_add){
        ...
        $new_image_data = array(
          ...
          'description' => $this->spider_replace4byte($image_new->description),
          'alt' => $this->spider_replace4byte($image_new->name),
    ...
    if($images_count != 0){
      ...
      foreach ($images_update as $image) {
        $save = $wpdb->update($wpdb->prefix . 'bwg_image', array(
          'order' => $image['order'],
          'slug' => $this->spider_replace4byte($image['slug']),
          'description' => $this->spider_replace4byte($image['description']),
          'alt' => $this->spider_replace4byte($image['alt']),
          'date' => $image['date']
          ), array('id' => $image['id']));
      }
    }
