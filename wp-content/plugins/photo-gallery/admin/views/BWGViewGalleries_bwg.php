...
class BWGViewGalleries_bwg {
  ...
  public function edit($id) {
    ...
    $gallery_type = ($row->gallery_type == 'instagram' || $row->gallery_type == 'instagram_post') ? 'instagram' : '';
    ...
    <?php wp_nonce_field( 'galleries_bwg', 'bwg_nonce' ); ?>
      ...
      <div style="float:right;">
        <input class="button-secondary" type="button" onclick="if (spider_check_required('name', 'Name') || bwg_check_instagram_gallery_input('<?php echo $option_row->instagram_client_id ?>') ) {return false;};
                                                     spider_set_input_value('page_number', '1');
      ...
      <table style="clear:both;">
        <tbody>
          ...
          <tr>
            ...
            <td>
              <select name="gallery_type" id="gallery_type" style="width:150px;" onchange="bwg_gallery_type('<?php echo $option_row->instagram_client_id ?>');">
                <option value="" <?php if ($gallery_type == '') echo 'selected="selected"'; ?>>Mixed</option>
                <option value="instagram" <?php if ($gallery_type == 'instagram') echo 'selected="selected"'; ?>>Instagram only</option> 
              </select>
        ...
        <tbody id="add_instagram_gallery"><!-- /*use this in javascript*/-->
          <tr id='tr_gallery_source' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label for="gallery_source">Instagram username: </label></td>
            <td><input type="text" id="gallery_source" name="gallery_source" value="<?php echo $gallery_source; ?>" size="64" /></td>

          </tr>
          <tr id='tr_autogallery_image_number' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label for="autogallery_image_number">Number of Instagram recent posts to add to gallery: </label></td>
          ...
          <tr id='tr_instagram_post_gallery' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label>Instagram embed type: </label></td>
            <td>
          ...
          <tr id='tr_update_flag' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label>Gallery autoupdate option: </label></td>
            <td>
          ...
          <tr id='tr_instagram_gallery_add_button' style='display:<?php echo (($gallery_type == 'instagram' && $images_count == 0) ? 'table-row': 'none'); ?>;'>
            ...
            <td>
              <input id="instagram_gallery_add_button" class="button-primary" type="button" onclick="add_instagram_gallery('<?php echo $option_row->instagram_client_id ?>');" value="Add Instagram Gallery" />    
            </td>
          </tr>
        </tbody>
        <tbody>      
  ...
  public function image_display($id) {
    ...
    ?>
      ...
      <div class="buttons_div_left">
        ...
        <input id="show_add_embed" class="button-primary" style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="button" onclick="jQuery('.opacity_add_embed').show(); jQuery('#add_embed_help').hide(); return false;" value="Embed Media" />
        <input id="show_bulk_embed" class="button-primary" style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="button" onclick="jQuery('.opacity_bulk_embed').show(); return false;" value="Bulk Embed" />
      </div>
      <div class="buttons_div_right">
        ...
        <input id="show_hide_weights"  class="button-secondary" type="button" onclick="spider_show_hide_weights();return false;" value="Hide order column" />
        <input class="button-primary" id='spider_setwatermark_button' style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="submit" onclick="spider_set_input_value('ajax_task', 'image_set_watermark');
                                                             spider_ajax_save('galleries_form');
                                                             return false;" value="Set Watermark" />
        <input class="button-secondary" id='spider_resize_button' style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="submit" onclick="jQuery('.opacity_resize_image').show(); return false;" value="Resize" />
        ...
        <input class="button-secondary" id='spider_reset_button' style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="submit" onclick="spider_set_input_value('ajax_task', 'image_recover_all');
                                                             spider_ajax_save('galleries_form');
        ...
        <input class="button-secondary spider_delete_button" style="<?php if($gallery_type != '' && $update_flag != '') {echo 'display:none';} ?>" type="submit" onclick="if (confirm('Do you want to delete selected items?')) {
      ...
      <div id="bulk_embed" class="opacity_bulk_embed bwg_bulk_embed">
        ...
        <input class="button-primary" type="button" onclick="bwg_bulk_embed('instagram', '<?php echo $option_row->instagram_client_id ?>');" value="Add to gallery" style="float:right; margin-left:5px;"/>
        <div class="spider_description"></div>
    		<table>
    		  <thead>
    			  <tr>
              <td class="spider_label_galleries"><label for="bulk_embed_from">Bulk embed from: </label></td>
              <td>
                <input type="radio" class="inputbox" id="bulk_embed_from_instagram" name="bulk_embed_from" onclick="jQuery('#instagram_bulk_params').show();" checked="checked" value="instagram" >
          ...
          <tbody id="instagram_bulk_params">
            <tr id='popup_tr_instagram_gallery_source' style='display:table-row'>
              <td class="spider_label_galleries"><label for="popup_instagram_gallery_source">Instagram username: </label></td>
            ...
            <tr id='popup_tr_instagram_image_number' style='display:table-row'>
              <td class="spider_label_galleries"><label for="popup_instagram_image_number">Number of Instagram recent posts to add to gallery: </label></td>
            ...
            <tr id='popup_tr_instagram_post_gallery' style='display:table-row'>
              ...
              <td>
                ...
                <label for="popup_instagram_post_gallery_0">Content</label>&nbsp;
                <input type="radio" class="inputbox" id="popup_instagram_post_gallery_1" name="popup_instagram_post_gallery" value="1" >
                <label for="popup_instagram_post_gallery_1">Whole post</label>
              </td>
            <tr>
    		  </tbody>
    		</table>
          ...
          if ($rows_data) {
            foreach ($rows_data as $row_data) {
              ...
              <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
                ...
                <td class="table_extra_large_col">
                  <input size="24" type="text" id="image_alt_text_<?php echo $row_data->id; ?>" name="image_alt_text_<?php echo $row_data->id; ?>" value="<?php echo $row_data->alt; ?> " <?php /* if($update_flag != '') {echo ' readonly="readonly" ';} */ ?> />
                  <?php if ($option_row->thumb_click_action != 'open_lightbox') { ?>
                  <input size="24" type="text" id="redirect_url_<?php echo $row_data->id; ?>" name="redirect_url_<?php echo $row_data->id; ?>" value="<?php echo $row_data->redirect_url; ?>" <?php /* if($update_flag != '') {echo ' readonly="readonly" ';} */?> />
                  <?php } ?>
                </td>
                <td class="table_extra_large_col">
                  <textarea cols="20" rows="2" id="image_description_<?php echo $row_data->id; ?>" name="image_description_<?php echo $row_data->id; ?>" style="resize:vertical;" <?php /* if($update_flag != '') {echo ' readonly="readonly" ';} */?>><?php echo $row_data->description; ?></textarea>
                </td>
                <td class="table_extra_large_col">
                ...
                <td class="table_big_col spider_delete_button" style="<?php if($gallery_type != '' && $update_flag != '') {echo 'display:none';} ?>"><a onclick="spider_set_input_value('ajax_task', 'image_delete');
                                                      spider_set_input_value('image_current_id', '<?php echo $row_data->id; ?>');
